...
### **v1.6.7** (2025-01-11)
- **UI/UX:** **메인 페이지 불필요한 안내 텍스트 제거**
  - **안내 텍스트 삭제**: "도서를 선택해주세요", "검색 후 도서를 선택하거나, 내 서재의 도서 제목을 클릭해주세요.", "책 제목을 입력하고 검색 버튼을 누르면, 검색 결과가 팝업으로 표시됩니다." 등의 불필요한 안내 메시지를 제거했습니다.
  - **UI 간소화**: 메인 페이지가 더 깔끔하고 현대적으로 보이도록 개선했습니다.
  - **공간 효율성**: 검색 폼과 다른 콘텐츠를 위한 공간을 확보했습니다.
  - **직관적 사용성**: 검색 폼의 플레이스홀더만으로도 충분히 직관적이므로 중복된 안내가 불필요함을 확인했습니다.
- **Refactor:** **컴포넌트 이름 명확화**
  - **BookModal → BookSearchListModal**: 검색 결과를 표시하는 모달의 이름을 더 명확하고 구체적으로 변경했습니다.
  - **함수명 일관성**: `openBookModal`, `closeBookModal`을 `openBookSearchListModal`, `closeBookSearchListModal`로 변경하여 일관성을 확보했습니다.
  - **코드 가독성 향상**: 컴포넌트의 역할과 기능을 이름에서 명확하게 파악할 수 있도록 개선했습니다.
- **UI/UX:** **리스트 뷰 레이아웃 일관성 개선**
  - **이미지 높이 정렬**: 가로가 넓은 이미지(예: "칵테일, 러브, 좀비")와 세로가 긴 이미지의 높이를 일치시켜 레이아웃 일관성을 향상시켰습니다.
  - **정보 영역 분리**: 상단 정보 블록(제목, 저자, 별점, 읽음 상태)과 하단 재고 블록(도서관 재고 정보)을 `<hr>` 구분선으로 명확히 분리했습니다.
  - **Flexbox 레이아웃 최적화**: 이미지 컨테이너가 상단 정보 블록의 높이에 맞춰지도록 Flexbox 속성을 조정하여 시각적 균형을 개선했습니다.
  - **이미지 비율 유지**: `object-contain`과 `max-h-full` 클래스를 적용하여 이미지가 잘리거나 왜곡되지 않으면서도 컨테이너 높이에 맞춰지도록 했습니다.
- **Fix:** **검색 기능 오류 수정**
  - **함수명 불일치 해결**: `BookModal` → `BookSearchListModal` 이름 변경으로 인한 `openBookModal is not a function` 에러를 수정했습니다.
  - **검색 기능 복구**: `useBookStore`의 `searchBooks` 함수에서 올바른 함수명(`openBookSearchListModal`)을 호출하도록 수정했습니다.
  - **도서 선택 기능 복구**: `selectBook` 함수에서 `closeBookSearchListModal`을 올바르게 호출하도록 수정했습니다.
- **UI/UX:** **BookDetails 스타일 균형 조정**
  - **폭 일관성**: 하단 카드뷰와 동일한 `max-w-4xl mx-auto` 적용하여 전체적인 레이아웃 균형을 개선했습니다.
  - **폰트 크기 최적화**: 과도하게 큰 폰트를 적절한 크기로 조정 (제목: 3xl→2xl, 저자: lg→base, 기타: md→sm).
  - **여백 최적화**: 패딩(p-6→p-4)과 그리드 간격(gap-6→gap-4)을 조정하여 공간 효율성을 향상시켰습니다.
  - **이미지 크기 조정**: 도서 표지 이미지 폭을 w-48→w-40으로 축소하여 정보 영역과의 균형을 개선했습니다.
  - **버튼 스타일 통일**: 모든 액션 버튼의 패딩(px-6→px-4)과 폰트 크기(text-sm)를 통일하여 일관성을 확보했습니다.
- **UI/UX:** **뷰 타입 이름 명확화**
  - **list view → card view**: 실제 표시되는 형태와 일치하도록 뷰 타입 이름을 변경했습니다.
  - **사용자 혼란 해소**: `list view`라는 혼란스러운 이름을 `card view`로 변경하여 사용자가 더 직관적으로 이해할 수 있도록 개선했습니다.
  - **일관성 향상**: `grid view`와 대칭적인 네이밍으로 UI의 일관성을 높였습니다.
  - **코드 가독성**: 변수명, 함수명, 주석을 모두 업데이트하여 개발자도 코드를 이해하기 쉽게 만들었습니다.
- **UI/UX:** **책 썸네일 배경 최적화**
  - **불필요한 회색 배경 제거**: 모든 뷰(grid view, card view, BookDetails, BookSearchListModal, MyLibraryBookDetailModal)에서 책 썸네일 이미지의 `bg-gray-700` 배경을 제거했습니다.
  - **이미지 가독성 향상**: 회색 배경이 제거되어 도서 표지 이미지가 더 선명하고 자연스럽게 표시됩니다.
  - **시각적 일관성**: 모든 도서 이미지가 동일한 스타일로 표시되어 UI의 일관성이 향상되었습니다.
  - **사용자 경험 개선**: 도서 표지의 원본 색상과 디자인이 더 잘 드러나 사용자가 도서를 식별하기 쉬워졌습니다.
- **UI/UX:** **도서 상세 정보 접근성 개선**
  - **제목 클릭 시 상세 정보 모달 열기**: card view와 grid view에서 도서 제목을 클릭하면 `MyLibraryBookDetailModal`이 열려 도서관 재고 정보를 포함한 상세 정보를 볼 수 있습니다.
  - **썸네일 클릭 시 상세 정보 모달 열기**: 도서 표지 이미지를 클릭해도 동일하게 상세 정보 모달이 열립니다.
  - **직관적인 상호작용**: 클릭 가능한 요소에 `cursor-pointer`와 `hover` 효과를 추가하여 사용자가 클릭할 수 있다는 것을 직관적으로 알 수 있습니다.
  - **툴팁 제공**: 제목과 썸네일에 "상세 정보 보기" 툴팁을 추가하여 기능을 명확하게 안내합니다.
  - **버튼 충돌 방지**: 카드 전체 클릭 대신 제목과 썸네일에만 클릭 이벤트를 적용하여 별점 변경, 읽음 상태 변경, 재고 새로고침 등의 버튼과 충돌하지 않습니다.

### **v1.6.6** (2025-01-11)
- **Fix:** **퇴촌도서관 상세페이지 웹 방화벽 차단 문제 해결**
    - **웹 방화벽 차단 문제**: 퇴촌도서관 상세페이지 URL(`resourcedetail/detail.do`)에 직접 접근 시 "Web firewall security policies have been blocked" 에러가 발생하는 문제를 해결했습니다.
    - **제목 기반 검색으로 우회**: 상세페이지 대신 검색 결과 페이지를 통해 접근하도록 변경하여 안정성을 향상시켰습니다.
    - **URL 구조 변경**: 기존 `generateLibraryDetailURL` 함수 대신 `toechonSearchUrl`을 사용하여 웹 방화벽 정책을 우회합니다.
    - **일관된 처리**: 모든 퇴촌도서관 링크가 동일한 방식으로 처리되도록 통일했습니다.
    - **사용자 경험 개선**: 검색 결과에서 해당 도서를 쉽게 찾을 수 있도록 안내합니다.
- **Docs:** **개발 문서 상세화**
    - **DEVELOPMENT.md 업데이트**: 퇴촌도서관 상세페이지 URL 방식의 문제점과 해결방안을 상세하게 문서화했습니다.
    - **트러블슈팅 가이드**: 웹 방화벽 차단 문제의 원인 분석, 해결 방안, 적용 효과를 체계적으로 정리했습니다.
    - **향후 개선 방향**: 도서관 API 연동, 캐싱 시스템, 사용자 피드백 등 향후 개발 계획을 제시했습니다.
