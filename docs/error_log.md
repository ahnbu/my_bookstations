...
---
### **9. 퇴촌도서관 상세페이지 웹 방화벽 차단 문제**
- **최초 발생일**: 2025-01-11
- **에러 현상**:
    - 퇴촌도서관 상세페이지 URL(`resourcedetail/detail.do`)에 직접 접근 시 "The request / response that are contrary to the Web firewall security policies have been blocked" 에러 발생.
    - 사용자가 도서관 재고 정보에서 퇴촌도서관 링크를 클릭했을 때 웹 방화벽에 의해 차단되어 접근할 수 없는 문제.
- **원인 분석**:
    - **웹 방화벽 정책**: 도서관 시스템의 보안 강화로 인한 외부 직접 접근 제한.
    - **URL 구조**: `resourcedetail/detail.do` 경로가 방화벽 정책에 위배됨.
    - **파라미터 접근**: `recKey`, `bookKey`, `publishFormCode`를 통한 직접 접근이 차단됨.
    - **보안 정책**: 도서관 시스템이 상세페이지의 직접 URL 접근을 차단하도록 설정됨.
- **해결 방법**:
    - **제목 기반 검색으로 우회**: 상세페이지 대신 검색 결과 페이지를 통해 접근하도록 변경.
    - **URL 구조 변경**: 기존 `generateLibraryDetailURL` 함수 대신 `toechonSearchUrl`을 사용하여 웹 방화벽 정책 우회.
    - **일관된 처리**: 모든 퇴촌도서관 링크가 동일한 방식으로 처리되도록 통일.
- **교훈 및 예방 조치**:
    - 외부 도서관 시스템과 연동할 때는 웹 방화벽이나 보안 정책을 고려해야 합니다.
    - 직접적인 상세페이지 접근이 차단될 경우, 검색 결과 페이지를 통한 우회 접근 방식을 고려해야 합니다.
    - 사용자 경험을 해치지 않는 선에서 안정성을 우선시하는 것이 중요합니다.
