...
- 광주시 종이책(KOLAS)과 광주시 시립 소장형 전자책은 같은 서버군 부하와 지연 영향을 받으므로 병렬 호출하지 않습니다.
- Worker는 광주시 종이책을 먼저 조회한 뒤, 남은 예산 안에서 시립 소장형 전자책을 조회합니다.
- 합산 예산은 15초, 각 endpoint 한도는 10초입니다. 종이책 조회 후 남은 시간이 1초 미만이면 시립 소장형 전자책은 생략됩니다.
- 생략 시 `siripEbook.errors.owned`에 `총 예산 부족으로 시립소장 전자책 조회 생략` 메시지가 들어갑니다. 저장 갱신에서는 이 상태를 소장형 오류로 취급하여 `stock_sirip_owned_*` 기존 값을 덮어쓰지 않습니다.
- 시립 구독형 전자책, 경기도 교육청, 경기도 전자도서관 조회는 광주시 종이책-소장형 예산과 독립적으로 진행됩니다.
...
태그 시스템은 "태그 정의"와 "책별 태그 적용 내역"을 서로 다른 저장 위치에 둡니다. 태그 관련 문제를 확인할 때는 먼저 어떤 데이터를 찾는지 구분해야 합니다.
...
| 확인 대상 | 관련 코드 |
| --- | --- |
| 사용자 설정 조회/저장 | `stores/useSettingsStore.ts` |
| 태그 생성/수정/삭제 | `stores/useSettingsStore.ts`의 `createTag`, `updateTag`, `deleteTag` |
| 책별 태그 추가/삭제/교체 | `stores/useBookStore.ts`의 `addTagToBook`, `removeTagFromBook`, `updateBookTags` |
| 태그 사용 권수 조회 | `stores/useBookStore.ts`의 `fetchTagCounts` |
| 태그 필터링 | `stores/useBookStore.ts`의 `filterLibraryByTags` |
| 타입 정의 | `types.ts`의 `UserSettings`, `BookDataSchema`, `Database` |
| 설정 UI | `components/SettingsModal.tsx`, `components/DefaultSettingsContent.tsx` |
| 책 상세 태그 UI | `components/MyLibraryBookDetailModal.tsx` |
| 내 서재 태그 표시/필터 UI | `components/MyLibrary.tsx`, `components/MyLibraryListItem.tsx` |
...
## 🔄 에러 포함 도서관 재고 데이터 자동 복구 및 수동 갱신 시스템

본 프로젝트는 데이터의 최신성과 정확성을 유지하기 위해 자동화된 백엔드 복구 시스템과 사용자 주도적인 수동 갱신 기능을 갖추고 있습니다.

### 1. 백엔드 자동 복구 (Cloudflare Worker Cron)

-   **목표**: 재고 조회 실패 등으로 인해 `stock_*` 컬럼 값이 `NULL`인 '에러 보유 책'들을 주기적으로 자동 복구합니다.
-   **실행 시점**: 매일 KST 새벽 2시 (`0 17 * * *` UTC).
-   **동작 파이프라인**:
    1.  **트리거**: Cloudflare Cron Triggers가 `scheduled` 핸들러를 실행합니다.
    2.  **대상 조회**: Worker가 Supabase의 `get_books_to_refresh()` DB 함수를 호출합니다. 이 함수는 `SECURITY DEFINER` 권한으로 RLS를 우회하여 모든 사용자의 '에러 보유 책' 목록을 효율적으로 가져옵니다.
    3.  **재고 조회**: Worker는 조회된 책 목록을 순회하며, 각 책의 재고 정보를 다시 크롤링/조회합니다. 서버 부하 분산을 위해 각 요청 사이에 **2초의 지연(delay)**을 둡니다.
    4.  **데이터베이스 업데이트**: 조회에 성공한 책에 한해 `user_library` 테이블의 `stock_*` 컬럼들을 `UPDATE`합니다.
-   **모니터링**: 모든 실행 과정과 결과(성공/실패 건수)는 Cloudflare 대시보드의 `Logs` 탭에 기록되어 관리자가 확인할 수 있습니다.

### 2. 프론트엔드 수동 갱신 (사용자 기능)

-   **목표**: 사용자가 '설정' 모달에서 직접 '에러 보유 책'들만 선택하여 즉시 재고를 갱신할 수 있는 편의 기능을 제공합니다.
-   **UI/UX 구현**:
    -   **동적 계산**: `useBookStore`는 서재 데이터가 변경될 때마다 '에러 보유 책'의 목록(`errorBooks`)과 개수(`errorBooksCount`)를 클라이언트에서 실시간으로 계산합니다.
    -   **옵션 제공**: '설정 > 저장갱신' 탭의 '재고 일괄 갱신' 드롭다운 메뉴에 '에러 보유 책 Only (xx권)' 옵션이 동적으로 표시됩니다. (에러 책이 1권 이상일 경우)
    -   **진행 상태 시각화**: 갱신 진행 시, 처리 중인 책의 제목을 실시간으로 표시하여 사용자에게 명확한 피드백을 제공합니다.
-   **데이터 흐름**:
    1.  사용자가 '에러 보유 책 Only' 옵션을 선택하고 갱신을 시작합니다.
    2.  `SettingsModal`은 `useBookStore`의 `errorBooks` 상태를 가져옵니다.
    3.  `bulkRefreshAllBooks` 액션을 호출하면서, `targetBooks` 인자로 `errorBooks` 목록을 전달합니다.
...
- **해결**: Cloudflare Worker(`library-checker/src/index.ts`)에서 각 도서관 `fetch` 요청에 `AbortSignal.timeout(15000)` (15초)을 설정했습니다. 특정 서버가 15초 내에 응답하지 않으면 해당 요청만 실패 처리하고 나머지 결과를 반환합니다.

### [유형 10-1] 광주시 종이책과 시립 소장형 전자책 병렬 호출 시 동반 타임아웃
- **증상**: 광주시 종이책(KOLAS)과 시립 소장형 전자책을 동시에 호출하면 둘 다 타임아웃되는 빈도가 높아짐.
- **원인**: 두 endpoint를 같은 요청 안에서 병렬로 때릴 때 대상 서버군의 지연·차단 영향이 겹침.
- **해결**: 광주시 종이책을 먼저 조회하고, 남은 예산 안에서 시립 소장형 전자책을 조회합니다. 두 조회의 합산 예산은 15초이며 각 endpoint는 최대 10초입니다. 남은 예산이 1초 미만이면 시립 소장형 전자책을 생략하고 기존 저장 재고를 보존합니다.
