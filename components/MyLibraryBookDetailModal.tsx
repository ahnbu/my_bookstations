...
import {
  ReadStatus, StockInfo, CustomTag, SelectedBook, GwangjuPaperResult, GwangjuPaperError, gyeonggiEbookResult, TagColor
} from '../types';
...
const StockDisplay: React.FC<StockDisplayProps> = ({
  ...
  const getStatus = (): StockStatus => {
    // [수정] 데이터가 아예 없는 경우(undefined)에만 에러(빨강)로 표시
    if (hasError && totalCount === undefined) return 'unavailable';

    // [수정] 에러가 있어도 데이터가 있다면(0 포함) 회색('none')으로 표시 (기존 데이터 활용)
  ...
  // [수정] (에러) 문구는 데이터가 아예 없을 때만 표시
  const showErrorMessage = hasError && totalCount === undefined;

  return (
    <div className="flex justify-between items-center">
      <span>{label}:</span>
      <div className="flex items-center gap-2">
        {showErrorMessage && <span className="font-medium text-red-400" title="정보 갱신 실패">(에러)</span>}
        <a
          href={searchUrl}
          target="_blank"
          ...
          title={titleText}
        >
          {finalTotalCount} / {finalAvailableCount}
        </a>
      </div>
    </div>
  );
...
const MyLibraryBookDetailModal: React.FC<MyLibraryBookDetailModalProps> = ({ bookId, onClose }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape' && !isEditingText && !useUIStore.getState().isJsonViewerModalOpen) {
        onClose();
      }
    };
            ...
            <div className="md:col-span-2 text-secondary">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <a
                  ...
                  className={`inline-flex items-center justify-center gap-2 px-5 py-2 bg-sky-500 text-white font-semibold rounded-lg transition-colors duration-300 ${!ebookLink ? 'opacity-50 cursor-not-allowed' : 'hover:bg-sky-600'
                    }`}
