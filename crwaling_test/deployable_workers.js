...
function parseGyeonggiHTML(html, libraryCode) {
  try {
    ...
    console.log(`경기도교육청(${branchName}) 파싱 시작...`);
    ...
    if (!searchResultsMatch) {
      ...
      console.log(`HTML에서 'search-results' 키워드 검색: ${html.includes('search-results') ? '발견됨' : '발견되지 않음'}`);
      return { library_name: `경기도교육청-${branchName}`, availability: [] };
    }
    const searchResultsHtml = searchResultsMatch[1];
    
    console.log(`경기도교육청(${branchName}): 검색 결과 영역 길이 ${searchResultsHtml.length}자`);
    ...
    const availability = bookItems.map((item, index) => {
      const bookHtml = item[0]; // 전체 매치 사용 (캡처 그룹이 아님)
