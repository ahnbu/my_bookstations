...
| 2026-08-18 16:30 | docs | onboarding | 데모 안내 띠 후속 조정 내역(색·문구·다크 연동)과 경량 완료검수보고서 추가 |
| 2026-08-18 16:29 | fix | onboarding | 데모 안내 띠 색을 톤다운하고 문구를 간결하게 바꾸며, 다크 모드가 OS 선호가 아닌 앱 테마를 따르도록 수정 |
| 2026-08-18 15:47 | docs | onboarding | 데모 안내 띠 상단 분리 구현계획(실행완료) 및 경량 계획·완료 검수보고서 추가 |
| 2026-08-18 15:35 | refactor | onboarding | 데모 예시 안내를 서재 본문에서 떼어내 페이지 최상단 고정 띠로 분리하고, 화면 분기 판정을 useDemoMode 훅으로 통합 |
| 2026-08-18 14:48 | docs | onboarding | 데모 서재 3차 개선 구현계획(실행완료) 및 계획·완료 검수보고서 추가 |
| 2026-08-18 14:48 | fix | onboarding | 데모 서재 그리드 레이아웃·열 수·정렬을 MyLibrary와 공유하고, 로그아웃·세션 만료 시 데모 대신 로그인 안내 화면 복원 |
| 2026-08-18 13:31 | docs | onboarding | 데모 서재 2차 개선 구현계획(실행완료) 및 plan-check 3회·done-check 검수보고서 추가 |
| 2026-08-18 13:31 | feat | onboarding | 데모 서재에 툴바 통합(검색·정렬·태그·좋아요·뷰 전환 실동작) + 도서 20권으로 재고 분포 현실화 + 웰컴 모달이 보이지 않는 채 body 스크롤만 잠그던 회귀 수정 |
| 2026-08-18 11:32 | docs | onboarding | 로그인 전 예시 서재 구현계획(실행완료) 및 plan-check 3회·done-check 검수보고서 추가 |
| 2026-08-18 11:31 | feat | onboarding | 로그인 전 화면에 예시 서재 10권 노출 + 초기 안내 메시지 기본 off — 비로그인 방문자가 빈 화면 대신 재고 배지가 붙은 서재를 보게 함 (정적 스냅샷, 백엔드 호출 없음) |
| 2026-08-08 20:02 | docs | auth | 구글 OAuth 클라이언트 삭제·재발급 기록 추가 + README 서비스정보에 유지 조건·소속 구글 프로젝트·재발급 경로 명시 (2026-04-23 삭제분을 새 클라이언트로 복구) |
| 2026-07-23 23:45 | docs | analysis | 마이북 git 커밋분석 xlsx 추가 |
| 2026-07-23 23:45 | docs | security | api-search 프록시 공개노출 조사메모 추가 (TTB 키 노출은 오탐, 트리셰이킹으로 번들 제거 확인) |
| 2026-07-23 23:42 | docs | backlog | 백로그 관리 규칙에 [저우선] 접두어 추가 (리뷰 완료·후순위 항목 표기) |
