...
### 👤 개인 서재 관리
- **🪟 로그인 전 예시 서재**: **이 브라우저에서 한 번도 로그인한 적 없는 방문자**에게만 도서 20권을 보여주고 검색·정렬·태그 필터·좋아요 필터·뷰 전환을 직접 조작해볼 수 있게 함. 재고가 많은 책부터 아예 없는 책까지 섞어 실제 서재에 가깝게 구성 (정적 스냅샷 `data/demoLibrary.json`, 읽기 전용 `components/DemoLibrary.tsx`). 레이아웃·그리드 열 수·정렬 규칙은 로그인 화면과 같은 코드를 공유하므로(`components/BookListContainer.tsx`, `hooks/useGridColumns.ts`, `utils/librarySort.ts`) 원본을 고치면 예시 서재에도 그대로 반영됨. **"예시입니다" 안내는 서재 안이 아니라 페이지 최상단 고정 띠(`components/DemoModeBanner.tsx`)에 둠** — 메타 안내와 서비스 본체를 섞지 않기 위함이며, 덕분에 서재 섹션은 로그인 화면과 구조가 같음
- **🔒 로그아웃·세션 만료 화면**: 로그인 이력이 있는 사용자가 로그아웃하거나 세션이 끊기면 예시 서재 대신 로그인 안내 화면을 보여줌(`components/LoggedOutLibraryNotice.tsx`). 내 서재 자리에 남의 책이 뜨는 혼란을 막음. 만료로 끊긴 경우에는 재로그인 안내 알림도 표시
...
- **초기 안내 메시지**: 기본값은 **표시 안 함**. 이 설정은 `localStorage`에만 저장되므로 관리자가 사용하는 브라우저에만 적용되고, 다른 방문자에게는 항상 기본값이 적용됨
...
### DB 아키텍처 리팩토링 및 UX 개선 (2025-11-10)
- **⚡️ 데이터 안정성 극대화**: API의 일시적인 실패에도 기존의 유효한 재고 정보가 유실되지 않도록 **데이터베이스 스키마를 리팩토링**하고 **"선택적 폴백(Fallback)"** 데이터 업데이트 전략을 도입했습니다. 이제 한번이라도 조회에 성공한 재고 정보는 API 오류에도 안전하게 보존됩니다.
- **🎨 UX 일관성 확보**: 내 서재 목록에서는 항상 안정적인 마지막 재고 정보를 표시하고(실패 시 빨간색 X), 상세 모달에서만 현재의 갱신 실패 상태를 알려주도록 역할을 분담하여 사용자 경험을 크게 개선했습니다.

### 백엔드 고도화 및 성능 최적화 (2025-11-08)
- **⚙️ TypeScript 전환**: Cloudflare Worker(백엔드) 코드를 TypeScript로 전면 전환하여 타입 안정성을 확보하고 런타임 에러 발생 가능성을 크게 줄였습니다.
- **🚀 API 응답 캐싱**: Cloudflare Cache API를 도입하여 반복적인 재고 조회 요청에 대한 응답 속도를 **수 초에서 수십 밀리초 단위로 99% 이상 단축**했습니다.
- **🛡️ 크롤링 안정성 강화**: 도서관 서버의 봇 탐지 정책에 대응하기 위해, 실제 브라우저 헤더를 정교하게 모방하는 2단계 요청 방식을 도입하고 세션 만료 시 자동 복구 로직을 구현하여 크롤링 성공률을 대폭 향상시켰습니다.

### 데이터 안정성 및 상태 관리 고도화 (2025-10-28)
...
- Cloudflare Worker의 광주시 계열 조회는 종이책(KOLAS)과 시립 소장형 전자책을 병렬로 호출하지 않습니다.
- 순서는 `광주시 종이책 -> 시립 소장형 전자책`이며, 두 조회의 합산 예산은 15초입니다.
- 각 조회는 최대 10초까지만 기다립니다. 종이책 조회가 지연되어 남은 예산이 1초 미만이면 시립 소장형 전자책은 생략하고 기존 저장 재고를 유지합니다.
- 시립 구독형 전자책, 경기도 교육청, 경기도 전자도서관 조회는 기존처럼 별도 흐름으로 진행됩니다.
