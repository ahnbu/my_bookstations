      ...
      try {
        ...
        if (title) {
            promises.push(
                ...
                searchGyeonggiEbookEducation(title, '10000009')  // 통합 (기존 title 사용)
            );
        }
...
async function searchGyeonggiEbookLibrary(searchText) {
  try {
    ...
    // 결과 통합 및 처리
    const ownedBooks = ownedResults.status === 'fulfilled' ? ownedResults.value : [];
    const subscriptionBooks = subscriptionResults.status === 'fulfilled' ? subscriptionResults.value : [];
    
    console.log(`소장형 도서: ${ownedBooks.length}권, 구독형 도서: ${subscriptionBooks.length}권`);
    ...
    const subscription = subscriptionBooks.map(book => ({
      title: book.title,
      author: book.author,
      publisher: book.publisher,
      isbn: book.isbn,
      totalCopies: null, // 구독형은 null
      availableCopies: null, // 구독형은 null
      isLoanable: book.available,
      type: book.type,
      library: book.library_name
    }));
    ...
    const availableCount = ownedBooks.filter(book => book.available).length + subscriptionBooks.length; // 구독형은 항상 대출가능

    console.log(`경기도 전자도서관 검색 완료: 총 ${totalStock}권, 대출가능 ${availableCount}권`);
...
async function searchSubscriptionBooks(query) {
  try {
    ...
    // --- 1단계: 동적 인증 토큰 생성 (로컬 환경 호환성 개선) ---
    // KST (UTC+9)를 기준으로 현재 시간 생성 - 더 정확한 시간대 처리
    const now = new Date();
    ...
    // Cloudflare Workers 환경과 로컬 환경 모두 호환되는 Base64 인코딩
    let dynamicToken;
    try {
      // 먼저 btoa 시도 (Cloudflare Workers 환경)
      if (typeof btoa !== 'undefined') {
        dynamicToken = btoa(tokenString);
      } else {
        // 로컬 Node.js 환경에서는 Buffer 사용
        dynamicToken = Buffer.from(tokenString).toString('base64');
      }
    } catch (error) {
      console.warn(`[경고] 기본 Base64 인코딩 실패, fallback 방식 사용: ${error.message}`);
      // 한글 등 특수문자 처리를 위한 fallback
      try {
        if (typeof btoa !== 'undefined') {
          dynamicToken = btoa(unescape(encodeURIComponent(tokenString)));
        } else {
          dynamicToken = Buffer.from(unescape(encodeURIComponent(tokenString))).toString('base64');
        }
      } catch (fallbackError) {
        throw new Error(`토큰 인코딩 실패: ${fallbackError.message}`);
      }
    }
    ...
    // --- 2단계: 요청 본문 및 헤더 구성 (로컬 환경 호환성 개선) ---
    const body = { 
      search: query, 
      searchOption: 1, 
      pageSize: 20, 
    ...
    console.log(`[성공] 구독형 도서 API 응답:`, JSON.stringify(data, null, 2));
    
    // 응답 데이터 유효성 검증
    if (!data || typeof data !== 'object') {
      throw new Error('API 응답이 올바르지 않습니다: 응답 데이터가 객체가 아닙니다.');
    }
    
    if (!data.bookSearchResponses) {
      console.warn('[경고] bookSearchResponses 필드가 없습니다. 전체 응답:', data);
    }
...
function parseSubscriptionResults(data, query) {
  try {
    ...
    // 응답 데이터 유효성 검증 강화
    if (!data) {
      console.log('구독형 도서 API 응답이 null 또는 undefined입니다.');
      return [];
    }
    
    if (typeof data !== 'object') {
      console.log(`구독형 도서 API 응답이 객체가 아닙니다: ${typeof data}`);
      return [];
    }
    
    // 문서에 따르면 응답 데이터는 bookSearchResponses 경로에 있음
    if (!data.bookSearchResponses) {
      console.log('bookSearchResponses 필드가 없습니다. 사용 가능한 필드들:', Object.keys(data));
      
      // 대안: 다른 가능한 필드들 확인
      const possibleFields = ['books', 'items', 'results', 'data'];
      let books = null;
      
      for (const field of possibleFields) {
        if (data[field] && Array.isArray(data[field])) {
          console.log(`대안 필드 발견: ${field}`);
          books = data[field];
          break;
        }
      }
    ...
    if (!Array.isArray(books)) {
      console.log(`도서 데이터가 배열이 아닙니다: ${typeof books}`);
      return [];
    }
    
    console.log(`구독형 도서 검색 결과: ${books.length}권 발견`);
    ...
    // 제목 기반으로 필터링 (대소문자 구분 없이, 더 유연한 매칭)
    const filteredBooks = books.filter(book => {
      if (!book || typeof book !== 'object') {
        console.log('잘못된 도서 객체:', book);
        return false;
      }
      
      const bookTitle = (book.title || book.bookTitle || book.name || '').toString().toLowerCase();
      const searchQuery = query.toLowerCase();
      
      // 정확한 매칭 또는 포함 관계 확인
      const isExactMatch = bookTitle === searchQuery;
      const isPartialMatch = bookTitle.includes(searchQuery);
      const isReversedMatch = searchQuery.includes(bookTitle);
      
      const isMatch = isExactMatch || isPartialMatch || isReversedMatch;
      
      if (isMatch) {
        console.log(`매칭된 도서: "${book.title || book.bookTitle || book.name}" (검색어: "${query}")`);
      }
      
      return isMatch;
    });

    console.log(`필터링 후 구독형 도서: ${filteredBooks.length}권`);

    // 도서 정보 매핑 개선
    return filteredBooks.map((book, index) => {
      const mappedBook = {
        type: '구독형',
        title: book.title || book.bookTitle || book.name || '전자책',
        author: book.author || book.writer || book.creator || '',
        publisher: book.publisher || book.pubCompany || book.pubCompanyName || '',
        isbn: book.isbn || book.isbn13 || book.isbn10 || '',
        totalCopies: book.totalCopies || book.totalCount || book.count || 0,
        availableCopies: book.availableCopies || book.availableCount || book.availableCount || 0,
        isAvailable: true, // 구독형은 기본적으로 대출 가능
        library: '경기도 전자도서관',
        detailUrl: book.detailUrl || book.url || book.link || '',
        coverImage: book.coverImage || book.cover || book.image || '',
        description: book.description || book.summary || book.content || '',
        category: book.category || book.genre || book.subject || '',
        publishDate: book.publishDate || book.pubDate || book.publicationDate || '',
        pageCount: book.pageCount || book.pages || book.page || 0
      };
    ...
    console.error('구독형 도서 결과 파싱 오류:', error);
